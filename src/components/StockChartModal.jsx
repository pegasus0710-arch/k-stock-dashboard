...
const MIN_DAY_OPTS = [{ label:'1일', days:1 }, { label:'3일', days:3 }, { label:'5일', days:5 }]
const MA_SETTINGS  = [
  { period:5,   color:'#f59e0b' }, { period:10,  color:'#10b981' },
  { period:20,  color:'#3b82f6' }, { period:60,  color:'#8b5cf6' },
...
function calcMA(data,p){
  return data.map((_,i)=>{
    ...
    return data.slice(i-p+1,i+1).reduce((s,d)=>s+(d.close||0),0)/p
  })
}
...
function CandleChart({ data, width, height, showMA }) {
  ...
  const maLines=showMA ? MA_SETTINGS.map(({period,color})=>{
    const vals=calcMA(data,period)
    const pts=vals.map((v,i)=>v?`${bx(i)},${py(v)}`:null).filter(Boolean)
  ...
  return (
    <div style={{position:'relative'}}>
      <svg ref={svgRef} width={width} height={height} onMouseMove={handleMouseMove}
        onMouseLeave={()=>setTooltip(null)} style={{display:'block',cursor:'crosshair'}}>
        {yTicks.map((v,i)=>(
          <g key={i}>
  ...
  return (
    <div className="smc-dart-overlay" onClick={e=>{if(e.target===e.currentTarget)onClose()}}>
      <div className="smc-dart-popup">
        <div className="smc-dart-header"><span>📋 {stock.name} 공시</span><button className="smc-close" onClick={onClose}>✕</button></div>
        {loading&&<div className="smc-dart-loading">불러오는 중…</div>}
        {!loading&&list.length===0&&(
          <div className="smc-dart-empty">
            {DART_API_KEY?'최근 공시 없음':'DART API 키 미설정'}<br/>
            <a href={`https://dart.fss.or.kr/dsab007/detailSearch.ax?textCrpNm=${encodeURIComponent(stock.name)}`}
              target="_blank" rel="noreferrer" className="smc-dart-link">DART에서 확인 →</a>
          </div>
        )}
        ...
        {selected&&(
          <div className="smc-dart-detail">
            ...
            <div className="smc-dart-detail-meta"><span>{selected.corp_name}</span><span>제출: {selected.flr_nm}</span></div>
            <a href={`https://dart.fss.or.kr/dsaf001/main.do?rcpNo=${selected.rcept_no}`}
              target="_blank" rel="noreferrer" className="smc-dart-link-btn">📄 DART 원문 보기 →</a>
          </div>
        )}
      </div>
    </div>
...
export default function StockChartModal({ stock, onClose }) {
  ...
  const [minDays,  setMinDays]  = useState(1)     // ✅ 분봉 기간
  ...
  useEffect(()=>{ const u=()=>{if(wrapRef.current)setChartWidth(wrapRef.current.clientWidth)}; u(); window.addEventListener('resize',u); return()=>window.removeEventListener('resize',u) },[])
  ...
  const fetchChart=useCallback(async()=>{
    ...
    try {
      const url=`/api/kiwoom?type=stock-chart&code=${stock.code}&period=${period}` +
                (period==='min' ? `&tic=${scope}&min_days=${minDays}` : '')
  ...
  },[stock?.code,period,scope,minDays])
  ...
  const pc=isUp?'#ef4444':isDown?'#3b82f6':'#64748b', sign=isUp?'+':''
  ...
  const infoItems=[
    ...
    // ✅ for_exh_rt = 외국인 보유비율 (server.py /price 응답에 포함)
      ...
      <div className="smc-overlay" onClick={e=>{if(e.target===e.currentTarget)onClose()}}>
        <div className="smc-modal">
          ...
          <div className="smc-controls">
            ...
            {period==='min'&&(
              <>
                ...
                <div className="smc-scope-wrap">
                  {MIN_DAY_OPTS.map(d=><button key={d.days} className={`smc-scope-btn ${minDays===d.days?'active':''}`} onClick={()=>setMinDays(d.days)}>{d.label}</button>)}
                </div>
              </>
            )}
          ...
          <div className="smc-chart-wrap" ref={wrapRef}>
            {loading&&<div className="smc-loading">⟳ 불러오는 중...</div>}
            ...
            {!loading&&!error&&chartData.length===0&&<div className="smc-empty">데이터 없음</div>}
          </div>
