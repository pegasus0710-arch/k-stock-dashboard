...
const STORAGE_KEY = 'kstock_briefing'

function loadSavedBriefing() {
  try {
    const raw = localStorage.getItem(STORAGE_KEY)
    if (!raw) return { text: '', savedAt: '' }
    const parsed = JSON.parse(raw)
    const today = new Date().toLocaleDateString('ko-KR')
    if (parsed.date !== today) { localStorage.removeItem(STORAGE_KEY); return { text: '', savedAt: '' } }
...
function saveBriefing(text) {
  try {
    ...
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ date: today, text, savedAt: hm }))
    return hm
  } catch { return '' }
}
      ...
      <div className="dash-header">
        <div className="dash-title-row">
          ...
          <div className="market-status-badge"
               style={{ background: marketStatus.color + '18', color: marketStatus.color, borderColor: marketStatus.color + '40' }}>
      ...
      <section className="dash-section">
        <div className="section-header">
          <div>
            ...
            {savedAt && (
              <span className="briefing-saved-time">오늘 {savedAt} 저장됨 · 새로고침해도 유지돼요</span>
            )}
          </div>
          <div className="briefing-btn-group">
            {briefing && (
              <button className="briefing-clear-btn" onClick={handleClearBriefing}>초기화</button>
            )}
            <button className={`ai-briefing-btn ${aiLoading ? 'loading' : ''}`}
                    onClick={handleAIBriefing} disabled={aiLoading}>
              {aiLoading
        ...
        {!briefing && !aiLoading && !aiError && (
          <div className="briefing-placeholder">
            ...
            <p className="placeholder-sub">받은 브리핑은 오늘 하루 자동 저장돼요 — 새로고침해도 유지됩니다</p>
          </div>
        )}
      ...
      <div className="dash-two-col">
        ...
        <section className="dash-section col-card">
          ...
          <div className="schedule-list">
            ...
            <a href="https://finance.naver.com/research/invest_list.naver" target="_blank" rel="noreferrer" className="schedule-link">📋 오늘 증권사 리포트 보기 →</a>
            <a href="https://dart.fss.or.kr/dsac999/mainY.do"              target="_blank" rel="noreferrer" className="schedule-link">📣 오늘 DART 공시 보기 →</a>
            <a href="https://finance.naver.com/sise/sise_quant.naver"       target="_blank" rel="noreferrer" className="schedule-link">🔥 거래량 상위 종목 →</a>
          </div>
        </section>
      </div>
