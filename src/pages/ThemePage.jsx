...
const THEMES = [
  {
    ...
    etf: [
      { name: 'KODEX 반도체', code: '091160' },
      { name: 'TIGER 반도체', code: '091230' },
    ],
    stocks: [
  ...
  },
  {
    ...
    etf: [
      { name: 'KODEX K-방산', code: '459580' },
      { name: 'TIGER 방산', code: '453810' },
    ],
    stocks: [
  ...
  },
  {
    ...
    etf: [
      { name: 'KODEX 조선', code: '139220' },
      { name: 'TIGER 조선', code: '395160' },
    ],
    stocks: [
  ...
  },
  {
    ...
    etf: [
      { name: 'KODEX 원자력', code: '445290' },
      { name: 'TIGER 원자력', code: '425420' },
    ],
    stocks: [
  ...
  },
  {
    ...
    etf: [
      { name: 'KODEX 2차전지', code: '305720' },
      { name: 'TIGER 2차전지', code: '364980' },
    ],
    stocks: [
      { name: 'LG에너지솔루션', code: '373220', desc: '글로벌 배터리 2위' },
      { name: '삼성SDI',        code: '006400', desc: '전고체 배터리 선도' },
      { name: 'SK온',           code: '285130', desc: '미국 배터리 공장 가동' },
  ...
  },
  {
    ...
    etf: [
      { name: 'KODEX 밸류업', code: '473190' },
      { name: 'TIGER 코리아밸류업', code: '474220' },
    ],
    stocks: [
...
async function fetchThemeAI(apiKey, theme) {
  ...
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: 'user',
        content: `오늘(${today}) 한국 증시 "${theme.label}" 테마(${theme.desc})에 대한 투자자용 분석을 작성해줘.
...
구체적인 수치와 종목명 포함해서 실용적으로 작성해줘. "실시간 데이터 없음" 같은 말은 절대 하지 마.`,
      }],
    }),
  })
...
function loadThemeAI() {
  try {
    ...
    if (date !== new Date().toLocaleDateString('ko-KR')) {
          ...
          <div className="theme-main">
            ...
            <div className="card-section theme-hero" style={{ borderLeft: `4px solid ${theme.color}` }}>
              <div className="theme-hero-top">
                ...
                <div className="theme-hero-actions">
                  {analysis && (
                    <button
                      className="btn-ai btn-ai--sm"
                      style={{ background: theme.color }}
                      onClick={handleAI}
                      disabled={aiLoading}
                    >
                      {aiLoading ? <><span className="btn-spinner" />분석중</> : '↺ 다시 분석'}
                    </button>
                  )}
                  {!analysis && (
                    <button
              ...
              <div className="theme-keywords">
                {theme.keywords.map(k => (
                  <span key={k} className="keyword-chip" style={{ background: theme.color + '14', color: theme.color }}>
                    {k}
                  </span>
                ))}
              </div>
            ...
            {!analysis && !aiLoading && !aiError && (
              <div className="ai-placeholder">
                ...
                <p>AI 분석 버튼을 눌러 <strong>{theme.label}</strong> 테마 분석을 받아보세요</p>
                <p className="sub">핵심 모멘텀 · 주목 종목 · 리스크 · 투자 전략을 정리해드려요<br/>분석 결과는 오늘 하루 저장됩니다</p>
              </div>
            )}
            ...
            <div className="card-section">
              <div className="section-title-row">
                ...
                <a
                  href={`https://finance.naver.com/sise/theme.naver`}
          ...
          <div className="theme-aside">
            ...
            <div className="card-section">
              ...
              <div className="aside-links">
                <a
                  href={`https://search.naver.com/search.naver?where=news&query=${encodeURIComponent(theme.label + ' 주식')}&sort=1`}
                  target="_blank" rel="noreferrer"
                  className="aside-link-item"
                >
                  <span>📰</span>
                  <span>{theme.label} 최신 뉴스</span>
                  <span className="aside-link-arrow">→</span>
                </a>
                <a
                  href={`https://dart.fss.or.kr/dsab007/detailSearch.ax`}
                  target="_blank" rel="noreferrer"
                  className="aside-link-item"
                >
                  <span>📋</span>
