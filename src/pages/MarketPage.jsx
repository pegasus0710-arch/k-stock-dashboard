...
async function fetchSectorAnalysis(apiKey, sector) {
  ...
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: 'user',
        content: `웹 검색으로 오늘(${today}) 한국 증시 "${sector}" 업종 최신 뉴스를 찾아보고 아래 형식으로 분석해줘.

검색어: ${sector} 업종 주식 뉴스 ${today}
    ...
    <div className="market-page">
      ...
      {activeTab === 'sector' && (
        <div className="tab-content">
                ...
                <div>
                  ...
                  <div className="sector-detail-links">
                    ...
                    <a href="https://finance.naver.com/sise/sise_group.naver?type=0"
                       target="_blank" rel="noreferrer" className="sector-link-btn">업종 종목 →</a>
                  </div>
                </div>
                <button
          ...
          {!selectedSector && (
            <div className="sector-guide">위에서 업종을 선택하면 AI 분석과 관련 링크를 바로 확인할 수 있어요</div>
          )}
        </div>
      )}
      ...
      {activeTab === 'supply' && (
        <div className="tab-content">
          ...
          <section className="market-section">
            <div className="section-label">수급 참고 지표</div>
            <div className="supply-ref-grid">
              {[
                {title:'📈 시장 전체 수급 흐름', desc:'코스피 투자자별 매매 동향', url:'https://finance.naver.com/sise/sise_index.naver?code=KOSPI'},
                {title:'🏛️ KRX 공식 수급 데이터', desc:'거래소 공식 통계', url:'https://data.krx.co.kr'},
                {title:'💻 프로그램 매매 현황', desc:'차익·비차익 프로그램 매매', url:'https://finance.naver.com/sise/program_list.naver'},
              ].map(s => (
                <a key={s.title} href={s.url} target="_blank" rel="noreferrer" className="supply-ref-card">
                  <div className="supply-ref-title">{s.title}</div>
                  <div className="supply-ref-desc">{s.desc}</div>
                </a>
              ))}
            </div>
          </section>
          <div className="supply-notice">
            <span>💡</span><span>키움 REST API 연동 후 외국인·기관 실시간 수급 데이터가 앱 내에 직접 표시됩니다</span>
          </div>
        </div>
      )}
